...
CREATE VIEW public.sce_comments_with_metadata AS
  SELECT t1.id, t1.created_at, t1.topic, t1.comment, t1.user_id, t1.parent_id, t1.mentioned_user_ids,
      (SELECT count(*) AS count
            FROM public.sce_comments c
            WHERE (c.parent_id = t1.id)) AS replies_count
  FROM public.sce_comments as t1 LEFT JOIN public.users as t2 ON t1.user_id = t2.id where t2.deactived = false;
...
CREATE VIEW public.sce_display_users AS
  SELECT t1.id, t2.deactived,
  COALESCE((t1.raw_user_meta_data ->> 'name'::text), (t1.raw_user_meta_data ->> 'full_name'::text), (t1.raw_user_meta_data ->> 'user_name'::text)) AS name,
  COALESCE((t1.raw_user_meta_data ->> 'avatar_url'::text), (t1.raw_user_meta_data ->> 'avatar'::text)) AS avatar
  FROM auth.users as t1 LEFT JOIN public.users as t2 ON t1.id = t2.id ;
...
--
-- Create boxes field in kaguya_images table
--

alter table users add column deactived boolean default false;

alter table kaguya_chapters add column language varchar;

alter table kaguya_chapters add column multilang varchar;
...
create table public.images_detected (
    id SERIAL primary key,
    created_at timestamp with time zone default now() not null,
    updated_at timestamp with time zone default now() not null,
    "additionData" json,
    "imagesId" bigint,
    constraint fk_images_detected_kaguya_images foreign key ("imagesId") references kaguya_images(id)
);

INSERT INTO 
    sce_reactions (type, label, url) 
VALUES 
    ('60fps_parrot', 'Parrot', 'https://emojis.slackmojis.com/emojis/images/1643514977/10031/60fps_parrot.gif?1643514977'),
    ('catjam', 'Catjam', 'https://emojis.slackmojis.com/emojis/images/1643514974/10003/catjam.gif?1643514974'),
    ('crycat', 'Cry cat', 'https://emojis.slackmojis.com/emojis/images/1643514761/7692/crycat.png?1643514761'),
